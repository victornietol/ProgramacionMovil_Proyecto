...
class BotonesPersonasWishlist extends StatefulWidget {
  ...
  final List<String> personas; // personas = lista de items
...
class _BotonesPersonasWishlistState extends State<BotonesPersonasWishlist> {
  // lista de widgets de los botones Personas
  List<Widget> botonesPersonas = [];
  List<String> listaTotalPersonas = [];
  List<String> listaActualPersonas = [];


  // lista de widgets de los botones Libros
  List<Widget> botonesLibros = [];
  Map<String,String> descripcionesLibros = {};
  String desc = "";
  List<String> listaTotalLibros = [];
  List<String> listaActualLibros = [];


  @override
  void initState() {
    super.initState();
    _leerPersonasyLibros().then((_) {
      _separarTipoItem();
      _crearWidgetPersonas();
      _crearWidgetLibros();
    });
  }

  Future<void> _leerPersonasyLibros() async {
    String leerTextP = await rootBundle.loadString("assets/texto/personas.txt");
    List<String> listaP = leerTextP.split(",");
    setState(() {
      listaTotalPersonas = listaP;
    });
    String leerTextL = await rootBundle.loadString("assets/libros/libros.txt");
    List<String> listaL = leerTextL.split(",");
    setState(() {
      listaTotalLibros = listaL;
    });
  }
  ...
  void _separarTipoItem() {
    for (int i=0; i<widget.personas.length; i++) {
      if (listaTotalPersonas.contains(widget.personas[i])) { // si contiene una persona se agrega a personas
        setState(() {
          listaActualPersonas.add(widget.personas[i]);
        });
      }
      if (listaTotalLibros.contains(widget.personas[i])) { // si contiene un libro se agrega a libros
        setState(() {
          listaActualLibros.add(widget.personas[i]);
        });
      }
    }
  }

  // crear widgets (botones) de las personas
  void _crearWidgetPersonas() {
    listaActualPersonas.sort();
    for (int i=0; i<listaActualPersonas.length; i++) {
      botonesPersonas.add(
          Container(
            height: 1,
            color: Colors.amberAccent.withOpacity(0.5),
      ...
      botonesPersonas.add(
        BotonInfoElevated(
            texto: listaActualPersonas[i],
            rutaImagen: "assets/imagenesPersonas/info/${listaActualPersonas[i]}/0.png",
            accion: () {
              showDialog(
                  context: context,
                  builder: (BuildContext context) {
                    return CuadroDialogo2(
                        nombreItem: listaActualPersonas[i],
                        contenido: Container(
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: <Widget>[
                                  ...
                                  onTap: () {
                                    showDialog(
                                        ...
                                        builder: (_) => Dialog(
                                          child: InteractiveViewer(
                                            child: Image.asset( // imagen que se abre en pabtalla completa
                                              "assets/imagenesPersonas/info/${listaActualPersonas[i]}/0.png",
                                              fit: BoxFit.contain,
                                            ),
                                          ),
                                        )
                                  ...
                                  onTap: () {
                                    showDialog(
                                        ...
                                        builder: (_) => Dialog(
                                          child: InteractiveViewer(
                                            child: Image.asset( // imagen que se abre en pabtalla completa
                                              "assets/imagenesPersonas/info/${listaActualPersonas[i]}/1.png",
                                              fit: BoxFit.contain,
                                            ),
                                          ),
                                        )
                                  ...
                                  onTap: () {
                                    showDialog(
                                        ...
                                        builder: (_) => Dialog(
                                          child: InteractiveViewer(
                                            child: Image.asset( // imagen que se abre en pabtalla completa
                                              "assets/imagenesPersonas/info/${listaActualPersonas[i]}/2.png",
                                              fit: BoxFit.contain,
                                            ),
                                          ),
                                        )
                                  ...
                                  onTap: () {
                                    showDialog(
                                        ...
                                        builder: (_) => Dialog(
                                          child: InteractiveViewer(
                                            child: Image.asset( // imagen que se abre en pabtalla completa
                                              "assets/imagenesPersonas/info/${listaActualPersonas[i]}/3.png",
                                              fit: BoxFit.contain,
                                            ),
                                          ),
                                        )
                                  ...
                                  onTap: () {
                                    showDialog(
                                        ...
                                        builder: (_) => Dialog(
                                          child: InteractiveViewer(
                                            child: Image.asset( // imagen que se abre en pabtalla completa
                                              "assets/imagenesPersonas/info/${listaActualPersonas[i]}/4.png",
                                              fit: BoxFit.contain,
                                            ),
                                          ),
                                        )
  ...
  // crear widgets (botones) de los libros
  void _crearWidgetLibros() {
    listaActualLibros.sort();
    for (int i=0; i<listaActualLibros.length; i++) {
      botonesLibros.add(
          Container(
            height: 1,
            color: Colors.amberAccent.withOpacity(0.5),
      ...
      botonesLibros.add(
        BotonInfoElevated(
            texto: listaActualLibros[i],
            rutaImagen: "assets/libros/imgLibro.png",
            accion: () {
              setState(() {
                _leeriInfoLibro2(listaActualLibros[i]);
              });
              showDialog(
                  context: context,
                  builder: (BuildContext context) {
                    return CuadroDialogo2( // CuadroDialogo1, CuadroDialogo2, CuadroDialogoSL dependiendo de que botones se quieren mostrar al final
                        nombreItem: listaActualLibros[i],
                        contenido: Container(
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: <Widget>[
  ...
  @override
  Widget build(BuildContext context) {
    List<Widget> botones = [...botonesPersonas,...botonesLibros];
    return ListView(
      children: botones,
    );
  }
        ...
        BotonInfoElevated(
            ...
            accion: () {
              ...
              showDialog(
                  context: context,
                  builder: (BuildContext context) {
                    return CuadroDialogo1( // CuadroDialogo1  o  CuadroDialogoSL dependiendo de que botones se quieren mostrar al final
                        nombreItem: widget.personas[i],
                        contenido: Container(
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
